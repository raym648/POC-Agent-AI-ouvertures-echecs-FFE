...
- ECO TSV public corpus (downloaded automatically)
...
Goal:
Generate ~150–200 structured chess opening entries suitable for:
- semantic retrieval
- embeddings
- Milvus ingestion
- chess opening RAG
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; WikichessDatasetBuilder/4.0)"
}
...
def extract_first_san_line(text: str) -> str:
    patterns = [
        r"(1\.\s?[A-Za-z0-9+=#-]+(?:\s+[A-Za-z0-9+=#-]+){1,15})",
        r"(1\.[A-Za-z0-9+=#-]+(?:\s+[A-Za-z0-9+=#-]+){1,15})",
    ]
...
def match_eco(opening_name: str, eco_rows: List[Dict]) -> Tuple[str, str, str]:
    target = opening_name.lower()

    for row in eco_rows:
        if row["opening"].lower() == target:
            return row["eco"], row["opening"], row["variation"]
...
def parse_article(url: str, eco_rows: List[Dict]) -> Optional[OpeningEntry]:
    ...
    eco, opening, variation = match_eco(opening_name, eco_rows)

    if not line_san:
        for row in eco_rows:
            if row["opening"].lower() == opening.lower():
                line_san = row["line_san"]
                break
